#!/usr/bin/env python3
"""
Build clips_kappa for the Cohen's Kappa inter-annotator agreement task.

Timeline per clip:  [5 s lead] → [2 s freeze, WITH numbered boxes] → [5 s tail]

The freeze frame is the annotated image (boxes already drawn by select_kappa_sample.py).
The expert watches context, sees the numbered boxes for 2 s, then more context.

Adapted from extract_iou_clips.py — inherits all special-case handling:
  - run_35: local frame numbering offset +8400
  - run_65: source video is wrong → clip built from the run's image sequence

Usage:
    python IAA/extract_kappa_clips.py
    python IAA/extract_kappa_clips.py --dry-run
"""
...
MANIFEST    = "./IAA/kappa/kappa_manifest.csv"
IMAGES_DIR  = "./IAA/kappa/images"
OUTPUT_DIR  = "./IAA/kappa/clips_kappa"
...
def build_clip_from_images(folder_name: str, frame_name: str,
    ...
    with tempfile.TemporaryDirectory(prefix="kappa_seq_") as tmp:
        tmp = Path(tmp)
...
def main():
    ...
    parser.add_argument("--freeze-duration", type=float, default=2.0,
                        help="Durée de la freeze frame avec les boîtes numérotées (défaut: 2 s)")
    ...
    with tempfile.TemporaryDirectory(prefix="kappa_freeze_") as tmp_dir:
        ...
        for row in rows:
            ...
            # The freeze frame is the annotated image (boxes already drawn)
            img_candidates = list(images_dir.glob(f"{anon_id}.*"))
            if not img_candidates:
                skipped.append((anon_id, "image annotée non trouvée")); n_skip += 1; continue
            ...
            if video_path is None:
                skipped.append((anon_id, f"vidéo introuvable: {row['video_path']}")); n_skip += 1; continue

            fps = get_fps(video_path, fps_cache)

            m = re.search(r"(\d+)", frame_name)
            if not m:
                skipped.append((anon_id, "impossible de parser l'index de frame")); n_skip += 1; continue
    ...
    print(f"\n{n_ok} clip(s) {'planifiés' if args.dry_run else 'écrits'}, {n_skip} ignorés.")
    if skipped:
        print("\nIgnorés :")
